...
FrameResource::FrameResource(VkDevice dev, VkPhysicalDeviceMemoryProperties memoryProperties, std::vector<VkDescriptorSet>& descriptorSets, uint32_t minAlignmentSize, uint32_t passCount, uint32_t objectCount, uint32_t materialCount) {
	device = dev;
	VkDeviceSize passConstantSize = passCount * sizeof(PassConstants);
	VkDeviceSize passDynamicSize = sizeof(PassConstants);
	if (minAlignmentSize > 0) {
		passDynamicSize = (VkDeviceSize)(((uint32_t)sizeof(PassConstants) + minAlignmentSize - 1) & ~(minAlignmentSize - 1));
		passConstantSize = passCount * passDynamicSize;
	}
	initBuffer(device, memoryProperties, passConstantSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, PassCB);
