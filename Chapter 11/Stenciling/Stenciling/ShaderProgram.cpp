...
ShaderResource::ShaderResource(VkDescriptorType descriptorType_, VkShaderStageFlags shaderStage_, size_t size_, size_t count_,size_t bufferCount_, bool isMapped_) {
	descriptorType = descriptorType_;
	shaderStage = shaderStage_;
	type = prtBuffer;
	buffer.size = size_;
	buffer.objCount = count_;
	buffer.bufferCount = bufferCount_;
...
void ShaderResources::AddDescriptorSets(std::vector<std::vector<DescriptorInfo>>& descriptorSetInfos) {
	std::unordered_map<VkDescriptorType, size_t> descriptorTypeMap;
	//std::unordered_map<VkDescriptorType, size_t> descriptorCountMap;
	for (auto& descriptorSetInfo : descriptorSetInfos) {
		for (auto& descriptorInfo : descriptorSetInfo) {
			if (descriptorTypeMap.find(descriptorInfo.descriptorType) == descriptorTypeMap.end()) {
				descriptorTypeMap.insert(std::pair<VkDescriptorType, size_t>(descriptorInfo.descriptorType, descriptorInfo.count));
			}
			else {
				descriptorTypeMap[descriptorInfo.descriptorType]+=(size_t)descriptorInfo.count;
			}
			/*if (descriptorCountMap.find(descriptorInfo.descriptorType) == descriptorCountMap.end()) {
				descriptorCountMap.insert(std::pair<VkDescriptorType, size_t>(descriptorInfo.descriptorType, descriptorInfo.count));
			}
			else {
				descriptorCountMap[descriptorInfo.descriptorType] += descriptorInfo.count;
			}*/
		}
	}
	...
	for (auto& pair : descriptorTypeMap) {
		//auto count = isBufferType(pair.first) ? passCount : 1;
		//size_t count = descriptorCountMap[pair.first];
		poolSizes.push_back({ pair.first,(uint32_t)(pair.second)});
		maxSet +=(uint32_t) pair.second;
		//maxSet = std::max((uint32_t)pair.second, maxSet);
	}

	maxSet = std::max(maxSet, (uint32_t)descriptorSetInfos.size());
	//size_t count = std::accumulate(descriptorCountMap.begin(), descriptorCountMap.end(),0u, [](uint32_t acc,std::pair<VkDescriptorType,size_t> a) {return (uint32_t) (acc+ (uint32_t)a.second); });
	descriptorPool = Vulkan::initDescriptorPool(vulkanContext, poolSizes,(uint32_t) maxSet);
	...
	for (auto& descriptorSetInfo : descriptorSetInfos) {
		...
		//bool imageOnly = true;
		//size_t count = 0;
		//for texture arrays, want to decouple amount of descriptors from pass count
		for (auto& descriptorInfo : descriptorSetInfo) {
			/*if (isBufferType(descriptorInfo.descriptorType))
				imageOnly = false;
			else
				count += descriptorInfo.count;*/
			descriptorBindings.push_back({ binding,descriptorInfo.descriptorType,1,descriptorInfo.shaderStage });
		}
		size_t count = std::accumulate(descriptorSetInfo.begin(), descriptorSetInfo.end(), 0u, [](uint32_t acc, DescriptorInfo a) {return (uint32_t)(acc + (uint32_t)a.count); });
...
void ShaderResources::AddBuffer(ShaderResource& res,/* uint32_t bufferCount, */ShaderResInternal& pipeRes) {
	...
	VkDeviceSize bufferSize = objectSize * res.buffer.objCount;
	VkDeviceSize range = bufferSize;
	VkDeviceSize totalSize = res.buffer.bufferCount * bufferSize;
...
void ShaderResources::UpdateDescriptors(/*uint32_t passCount*/) {
	...
	for(size_t i=0;i<writeInfos.size();++i){
		...
		std::vector<VkWriteDescriptorSet> descriptorWrites(writeInfos[i].size());
		for (uint32_t j = 0; j < writeInfos[i].size(); ++j) {
			if (isBufferType(writeInfos[i][j].descriptorType))
				descriptorWrites[j] = { VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,descriptorSets[j][j],0,0,1,writeInfos[i][j].descriptorType,nullptr,&writeInfos[i][j].buffer,nullptr };
			else
				descriptorWrites[j] = { VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,nullptr,descriptorSets[i][j],0,0,1,writeInfos[i][j].descriptorType,&writeInfos[i][j].image,nullptr,nullptr };
		}
...
void ShaderResources::AddShaderResources(std::vector<std::vector<ShaderResource>>& pipelineResData/*, uint32_t passCount*/) {
	...
	for (auto& pipelineResource : pipelineResData) {
		...
		for (auto& res : pipelineResource) {
			...
			case prtBuffer:
				...
				count = res.count = res.buffer.bufferCount;
				break;
			case prtTexture:
				AddTexture(res, pipeRes);
				res.count = res.textures.count;
				break;
			case prtImageData:
	...
	AddDescriptorSets(descriptorSetInfos);

	for (auto& pipelineResource : pipelineResources) {
		size_t count = 1/*passCount*/;
		if (isBufferType(pipelineResource.descriptorType))
			count = pipelineResource.buffer.totalSize / pipelineResource.buffer.passSize;
		else
			count = pipelineResource.textures.count;
		
		
		std::vector<DescriptorWriteInfo> descriptorWrites(count);
		...
		for (size_t i = 0; i < count; ++i) {
			...
			if (isBufferType(pipelineResource.descriptorType)) {
				...
				Vulkan::Texture texture;
				if (pipelineResource.textures.count == count) {
