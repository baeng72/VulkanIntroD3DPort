...
struct DescriptorInfo {
	...
	size_t count{ 1 };//set to different value for texture arrays.
};
...
enum ShaderResourceType { prtBuffer, prtTexture, prtImageData };
struct ShaderResource :public DescriptorInfo {
	ShaderResourceType type{ prtBuffer };
	...
	ShaderResource(VkDescriptorType descriptorType_, VkShaderStageFlags shaderStage_, size_t size_, size_t objCount_,size_t bufferCount_, bool isMapped_);/* {
	...
	ShaderResource(VkDescriptorType descriptorType_, VkShaderStageFlags shaderStage_, size_t width_, size_t height_, size_t pixelSize, uint8_t* pixels_, bool enableLod = false);/* {
		descriptorType = descriptorType_;
		shaderStage = shaderStage_;
		type = prtTexture;
	...
	ShaderResource(VkDescriptorType descriptorType_, VkShaderStageFlags shaderStage_, Vulkan::Texture* pTextures_, size_t textureCount);
	~ShaderResource() {}
};



struct ShaderResInternal {
	...
	ShaderResInternal() { descriptorType = VK_DESCRIPTOR_TYPE_MAX_ENUM; }
	ShaderResInternal(VkDescriptorType descriptorType_, Vulkan::Buffer& buffer_, void* ptr, VkDeviceSize objectSize, VkDeviceSize passSize, VkDeviceSize totalSize);/* {
		descriptorType = descriptorType_;
	...
	ShaderResInternal(VkDescriptorType descriptorType_, TextureArray& textureArray) {
		descriptorType = descriptorType_;
...
class ShaderResources {
	...
	//		buffer.totalSize = totalSize;
	//	}*/
	//	ShaderResInternal(VkDescriptorType descriptorType_, std::vector<Vulkan::Texture>& textures_) {
	//		descriptorType = descriptorType_;
	//		textures = textures_;
	//	}
	//	
	...
	std::vector<ShaderResInternal> pipelineResources;
	std::vector<std::vector< VkDescriptorBufferInfo>> bufferInfo;
	std::vector<std::vector<DescriptorWriteInfo>> writeInfos;
	void AddDescriptorSet(std::vector<DescriptorInfo>& descriptorSetInfo, std::unordered_map<VkDescriptorType, size_t>& descriptorTypeMap);
	void AddDescriptorSets(std::vector<std::vector<DescriptorInfo>>& descriptorSetInfos);/* {
	...
	void AddBuffer(ShaderResource& res, /*uint32_t bufferCount,*/  ShaderResInternal & pipeRes); //{
//		Vulkan::Buffer buffer;
...
//	}
	void AddTexture(ShaderResource& res, ShaderResInternal& pipeRes);/* {

	}*/
	void AddImageData(ShaderResource& res, ShaderResInternal& pipeRes);
...
public:
	...
	//	}*/
	//}
	void AddShaderResources(std::vector<std::vector<ShaderResource>>& pipelineResData/*, uint32_t passCount*/); // {
	...
	//}
	ShaderResources(VulkanContext& context);// :device(device_), deviceProperties(deviceProperties_), memoryProperties(memoryProperties_) {
	//}
	...
	const ShaderResInternal& GetShaderResource(size_t i) const { return pipelineResources[i]; }
	const VkDescriptorSet GetDescriptorSet(size_t i, size_t pass)const { return descriptorSets[i][pass]; }
};
