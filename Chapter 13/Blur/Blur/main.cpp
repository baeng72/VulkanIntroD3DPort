...
public:
	...
	BlurApp(const BlurApp& hrs) = delete;
...
BlurApp::BlurApp(HINSTANCE hInstance) :VulkApp(hInstance) {
...
bool BlurApp::Initialize() {
	...
	blurFilter = std::make_unique<BlurFilter>(mDevice,mComputeQueue, mClientWidth, mClientHeight, PREFERRED_IMAGE_FORMAT);
...
glm::vec3 BlurApp::GetHillsNormal(float x, float z)const
{
	// n = (-df/dx, 1, -df/dz)
	glm::vec3 n(
...
void BlurApp::Update(const GameTimer& gt) {
	VulkApp::Update(gt);//should wait on Fence, better check it
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void BlurApp::UpdateMaterialsCBs(const GameTimer& gt) {
...
void BlurApp::OnResize() {
	...
	if (blurFilter != nullptr) {
		blurFilter->OnResize(mClientWidth, mClientHeight);
	}
}
...
void BlurApp::DrawRenderItems(VkCommandBuffer cmd, const std::vector<RenderItem*>& ritems) {
...
int main() {
	...
	try
	{
		BlurApp theApp(GetModuleHandle(NULL));
