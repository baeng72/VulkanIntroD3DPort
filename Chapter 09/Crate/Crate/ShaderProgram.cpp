...
ShaderResource::ShaderResource(VkDescriptorType descriptorType_, VkShaderStageFlags shaderStage_, Vulkan::Texture& texture_) {
	descriptorType = descriptorType_;
	shaderStage = shaderStage_;
	type = prtTexture;
	texture = texture_;
...
ShaderResources::ShaderResInternal::ShaderResInternal(VkDescriptorType descriptorType_, Vulkan::Buffer& buffer_, void* ptr, VkDeviceSize objectSize, VkDeviceSize passSize, VkDeviceSize totalSize) {
	descriptorType = descriptorType_;
...
void ShaderResources::AddTexture(ShaderResource& res, ShaderResInternal& pipeRes) {
	...
	VkDescriptorType descriptorType = res.descriptorType;
	Vulkan::Texture& texture = res.texture;
	pipeRes = { descriptorType,texture };
}
	...
	for (auto& pipelineResource : pipelineResources) {
		...
		for (uint32_t i = 0; i < passCount; ++i) {
			...
			if (isBufferType(pipelineResource.descriptorType)) {
				...
				descriptorWrites[i].image.imageView = pipelineResource.texture.imageView;
				descriptorWrites[i].image.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
				descriptorWrites[i].image.sampler = pipelineResource.texture.sampler;
			}
		}
...
ShaderResources::~ShaderResources() {
	for (auto& pipelineRes : pipelineResources) {
		...
		if (isBufferType(pipelineRes.descriptorType)) {
			...
			vkDestroySampler(vulkanContext, pipelineRes.texture.sampler, nullptr);
			Vulkan::cleanupImage(vulkanContext, pipelineRes.texture);
		}
