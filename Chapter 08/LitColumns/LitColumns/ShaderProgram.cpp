...
ShaderResource::ShaderResource(VkDescriptorType descriptorType_, VkShaderStageFlags shaderStage_, size_t width_, size_t height_, uint8_t* pixels_) {
	descriptorType = descriptorType_;
	shaderStage = shaderStage_;
	type = prtTexture;
...
ShaderResources::ShaderResources(VkDevice device_, VkPhysicalDeviceProperties& deviceProperties_, VkPhysicalDeviceMemoryProperties memoryProperties_) :device(device_), deviceProperties(deviceProperties_), memoryProperties(memoryProperties_) {
}
