...
class CubeMapApp : public VulkApp {
...
CubeMapApp::CubeMapApp(HINSTANCE hInstance) :VulkApp(hInstance) {
...
void CubeMapApp::BuildDescriptors() {
	...
	DescriptorSetBuilder::begin(descriptorSetPoolCache.get(), descriptorSetLayoutCache.get())
		.AddBinding(0, VK_DESCRIPTOR_TYPE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
		.AddBinding(1, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, VK_SHADER_STAGE_FRAGMENT_BIT, (uint32_t)1)
	...
	cubeMapDescriptor = std::make_unique<VulkanDescriptor>(mDevice, descriptor5);
	{
		...
		imageInfo.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo.imageView = ct;

		
		DescriptorSetUpdater::begin(descriptorSetLayoutCache.get(), descriptorLayout5, descriptor5)
			.AddBinding(0, VK_DESCRIPTOR_TYPE_SAMPLER, &sampInfo)
			.AddBinding(1, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, &imageInfo, (uint32_t)1)
			.update();
	}
...
void CubeMapApp::UpdateMaterialsBuffer(const GameTimer& gt) {
...
void CubeMapApp::Draw(const GameTimer& gt) {
	...
	if (mIsWireframe) {
		...
		pvkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, *cubeMapPipelineLayout, 2, 1, &descriptor5, 0, 0);//bind PC data once
...
void CubeMapApp::DrawRenderItems(VkCommandBuffer cmd, VkPipelineLayout layout, const std::vector<RenderItem*>& ritems) {
	auto& ub = *uniformBuffer;
...
int main() {
	...
	try
	{
		CubeMapApp theApp(GetModuleHandle(NULL));
